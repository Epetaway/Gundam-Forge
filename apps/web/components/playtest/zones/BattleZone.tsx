...
export function BattleZone({
  ...
  return (
    <div
      ...
      className={`border-2 rounded-lg bg-surface-elevated/70 p-4 h-full flex flex-col transition-colors ${
      ...
      <div className="flex-1 grid gap-3 overflow-y-auto max-h-96">
        {units.length > 0 ? (
          units.map((unit) => (
            <div
              key={unit.instanceId}
              className="flex items-center gap-3 p-3 rounded-lg border-2 border-border bg-gradient-to-b from-surface to-surface-elevated hover:border-cobalt-500 transition-all duration-200 cursor-pointer"
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-bold text-steel-300 truncate">
                ...
                {unit.state === 'rest' && (
                  <div className="mt-1 text-sm text-steel-600 italic">RESTING</div>
                )}
              </div>
            </div>
          ))
